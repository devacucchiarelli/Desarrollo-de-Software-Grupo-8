...
function Home({ usuario }) {
  ...
  const handleChange = (e) => {
    ...
    setFormData(prev => {
        ...
        if (name === 'formato') {
            newState.cantidadEquipos = value === 'eliminatoria' 
                ? (prev.cantidadEquipos || '16') 
                : (value === 'liga' ? prev.cantidadEquipos || '' : '');
        }
        return newState;
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.formato === 'eliminatoria' && (!cantidadEquiposNum || ![8, 16, 32].includes(cantidadEquiposNum))) {
        setError("Para Eliminatoria, equipos debe ser 8, 16 o 32."); 
        return;
    }
    if (formData.formato === 'liga' && (!cantidadEquiposNum || cantidadEquiposNum < 4 || cantidadEquiposNum > 30)) {
        setError("Para Liga, equipos debe ser entre 4 y 30."); 
        return;
    }
    if (!formData.formato) { 
        setError("Debes seleccionar un formato."); 
        return; 
    }
    ...
    try {
      const dataToSend = {
          ...
          ...((formData.formato === 'eliminatoria' || formData.formato === 'liga') && 
             formData.cantidadEquipos && 
             { cantidad_equipos: cantidadEquiposNum })
      };
      ...
      if (!response.ok) { 
          const errorData = await response.json().catch(() => ({ 
              error: 'Error desconocido al guardar torneo' 
          })); 
  ...
  const handleEdit = (e) => {
    ...
    setFormData({
      ...
      cantidadEquipos: torneo.cantidad_equipos?.toString() || "",
    });
    setEditando(true);
    setMostrarForm(true);
  };
  ...
  const handleDelete = async (e) => {
    e.stopPropagation(); 
    if (window.confirm(`¿Seguro querés eliminar el torneo "${torneo.nombre_torneo}"?`)) { 
        try { 
            ...
            if (!response.ok) { 
                const errorData = await response.json().catch(() => ({ 
                    error: 'Error desconocido al eliminar torneo' 
                })); 
  ...
  const handleCardClick = () => {
      ...
      if (torneo.formato === 'eliminatoria') {
          ...
          navigate(`/torneo/${torneo.id_torneo}/tabla`);
      }
  };
        ...
        <div className="main-content-wrapper">
          ...
          {torneo && !mostrarForm && (
              <div 
                  ...
                  title={`Click para ver ${torneo.formato === 'liga' ? 'tabla' : 'fixture'}`}
              >
                 ...
                 <div className="torneo-details">
                     ...
                     {torneo.cantidad_equipos && (
                         <p><span className="detail-label">Equipos:</span> {torneo.cantidad_equipos}</p>
                     )}
                 </div>
                 {/* Solo administradores pueden editar/eliminar torneos */}
                 {isAdmin && (
          ...
          {/* Solo administradores ven el formulario */}
          {isAdmin && mostrarForm && (
            <form onSubmit={handleSubmit} className="form-card">
              ...
              <div className="form-inputs-container">
                  <input 
                      ...
                      className="form-input-nombreTorneo" 
                      required 
                  />
                  ...
                  <select
                      ...
                      title={editando && formatoOriginalAlEditar === 'eliminatoria' 
                          ? "No se puede cambiar el formato una vez creado como Eliminatoria" 
                          : "Seleccionar formato"}
                  >
                  ...
                  {formData.formato === 'eliminatoria' && (
                      ...
                      >
                          <option value="" disabled={formData.cantidadEquipos !== ""}>
                              Nº equipos (Eliminatoria)
                          </option>
